...
# partition function: hacking the metadata.properties in generated segments: 
column.event_id.partitionFunction = Murmur
column.event_id.numPartitions = 1
column.event_id.partitionValues = 0

# Drop dead servers/broker/etc;  
check zookeeper ideal state and external view; controller;  if needed, edit the zk's ideal state (e.g., segment asignments to servers)
clean the server's data directory;  delete pinot-all.log 

# delete segments
From controller UI or POST of /segments/tableName
